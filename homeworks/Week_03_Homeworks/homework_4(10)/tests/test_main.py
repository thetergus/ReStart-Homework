...
def test_PUT_good_1_____________():
...
def test_PUT_good_2_____________():
...
def test_PUT_bad_author_name____():
...
def test_PUT_bad_author_code____():
    response = client.put("/books/1222/", json=
        {
            "book_name": "Lettuce Attempt",
            "book_author": {
...
def test_PUT_bad_publishing_year(): #
    response = client.put("/books/1222/", json=
        {
            "book_name": "Lettuce",
            "book_author": {
...
def test_GET_single_good________():
...
def test_GET_single_bad_________():
    response = client.get("/books/Ranting/")
    assert response.status_code==404


def test_GET_all_good_exists____(): #there is no validation scripted  for this
...
def test_DELETE_bad_____________():    #Doing Bad one first since we currently have a book
    response = client.delete("/books/Vuvuzelas/")
    assert response.status_code==404   


def test_DELETE_good____________(): 
    response = client.delete("/books/DaVinci%20Code/")
    assert response.status_code==200   


def test_DELETE_good_confirm____(): 
    response = client.delete("/books/DaVinci%20Code/")
    assert response.status_code==404   


def test_DELETE_good_2__________(): 
    response = client.delete("/books/The%20Lord%20of%20the%20Rigns/")
    assert response.status_code==200   


def test_DELETE_good_confirm_2__(): 
    response = client.delete("/books/The%20Lord%20of%20the%20Rigns/")
    assert response.status_code==404       
...
def test_GET_all_good_no_data___():
